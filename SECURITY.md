...
If you believe you have found a security vulnerability in `z4j-apscheduler`,
...
This adapter runs with the application's APScheduler jobstore access. An
authenticated brain can invoke the advertised pause, resume, trigger, and
delete controls. Job-id resolution, scheduler API calls, and schedule metadata
sent to the brain are therefore package-specific security surfaces; transport,
redaction, and authorization policy remain owned by `z4j-core` and the brain.
